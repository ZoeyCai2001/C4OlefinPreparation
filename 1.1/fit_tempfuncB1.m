...
%    工作表: 性能数据表
%
% 由 MATLAB 于 2021-09-10 13:27:52 自动生成
...
% 导入数据
B1 = readtable("D:\2021国赛\CUMCM2021Probelms\B\附件1.xlsx", opts, "UseExcel", false);
...
%% Fit: 'B1temp-乙醇'.
[xData, yData] = prepareCurveData( B1temp, B1yichun );

% Set up fittype and options.
ft = fittype( 'exp1' );
opts = fitoptions( 'Method', 'NonlinearLeastSquares' );
opts.Display = 'Off';
opts.StartPoint = [0.00484263416311945 0.0230971121222815];

% Fit model to data.
[fitresult{1}, gof(1)] = fit( xData, yData, ft, opts );

% Plot fit with data.
figure( 'Name', 'B1temp-乙醇' );
h = plot( fitresult{1}, xData, yData );
legend( h, 'B1yichun vs. B1temp', 'B1temp-乙醇', 'Location', 'NorthEast', 'Interpreter', 'none' );
% Label axes
...
%% Fit: 'B1temp-c4'.
[xData, yData] = prepareCurveData( B1temp, B1c4 );
...
% Plot fit with data.
figure( 'Name', 'B1temp-c4' );
h = plot( fitresult{2}, xData, yData );
legend( h, 'B1c4 vs. B1temp', 'B1temp-c4', 'Location', 'NorthEast', 'Interpreter', 'none' );
% Label axes
xlabel( '温度', 'Interpreter', 'none' );
