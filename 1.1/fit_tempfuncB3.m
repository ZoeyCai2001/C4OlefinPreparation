...
%    工作表: 性能数据表
%
% 由 MATLAB 于 2021-09-10 13:42:51 自动生成
...
% 导入数据
B3 = readtable("D:\2021国赛\CUMCM2021Probelms\B\附件1.xlsx", opts, "UseExcel", false);
...
%% Fit: 'B3temp-乙醇'.
[xData, yData] = prepareCurveData( B3temp, B3yichun );

% Set up fittype and options.
ft = fittype( 'exp1' );
opts = fitoptions( 'Method', 'NonlinearLeastSquares' );
opts.Display = 'Off';
opts.StartPoint = [0.000286398984345033 0.02813893901104];

% Fit model to data.
[fitresult{1}, gof(1)] = fit( xData, yData, ft, opts );

% Plot fit with data.
figure( 'Name', 'B3temp-乙醇' );
h = plot( fitresult{1}, xData, yData );
legend( h, 'B3yichun vs. B3temp', 'B3temp-乙醇', 'Location', 'NorthEast', 'Interpreter', 'none' );
% Label axes
...
%% Fit: 'B3temp-c4'.
[xData, yData] = prepareCurveData( B3temp, B3c4 );
...
% Plot fit with data.
figure( 'Name', 'B3temp-c4' );
h = plot( fitresult{2}, xData, yData );
legend( h, 'B3c4 vs. B3temp', 'B3temp-c4', 'Location', 'NorthEast', 'Interpreter', 'none' );
% Label axes
xlabel( '温度', 'Interpreter', 'none' );
