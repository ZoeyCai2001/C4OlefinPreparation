...
%    工作表: 性能数据表
%
% 由 MATLAB 于 2021-09-10 10:45:39 自动生成
...
% 导入数据
A9 = readtable("D:\2021国赛\CUMCM2021Probelms\B\附件1.xlsx", opts, "UseExcel", false);


%% 清除临时变量
clear opts
%%
A9array = table2array(A9);
A9temp = A9array(:,1);
A9yichun = A9array(:,2);
A9c4 = A9array(:,3);
%% Initialization.
...
%% Fit: 'A9temp-乙醇'.
[xData, yData] = prepareCurveData( A9temp, A9yichun );

% Set up fittype and options.
ft = fittype( 'exp1' );
opts = fitoptions( 'Method', 'NonlinearLeastSquares' );
opts.Display = 'Off';
opts.StartPoint = [0.0142436915889248 0.019774256220127];

% Fit model to data.
[fitresult{1}, gof(1)] = fit( xData, yData, ft, opts );

% Plot fit with data.
figure( 'Name', 'A9temp-乙醇' );
h = plot( fitresult{1}, xData, yData );
legend( h, 'A9yichun vs. A9temp', 'A9temp-乙醇', 'Location', 'NorthEast', 'Interpreter', 'none' );
% Label axes
...
%% Fit: 'A9temp-c4'.
[xData, yData] = prepareCurveData( A9temp, A9c4 );
...
% Plot fit with data.
figure( 'Name', 'A9temp-c4' );
h = plot( fitresult{2}, xData, yData );
legend( h, 'A9c4 vs. A9temp', 'A9temp-c4', 'Location', 'NorthEast', 'Interpreter', 'none' );
% Label axes
xlabel( '温度', 'Interpreter', 'none' );
