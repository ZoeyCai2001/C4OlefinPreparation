...
%    工作表: 性能数据表
%
% 由 MATLAB 于 2021-09-10 14:02:18 自动生成
...
% 导入数据
B5 = readtable("D:\2021国赛\CUMCM2021Probelms\B\附件1.xlsx", opts, "UseExcel", false);
...
%% Fit: 'B5temp-乙醇'.
[xData, yData] = prepareCurveData( B5temp, B5yichun );

% Set up fittype and options.
ft = fittype( 'exp1' );
opts = fitoptions( 'Method', 'NonlinearLeastSquares' );
opts.Display = 'Off';
opts.StartPoint = [0.0142996365914178 0.0201064154989151];

% Fit model to data.
[fitresult{1}, gof(1)] = fit( xData, yData, ft, opts );

% Plot fit with data.
figure( 'Name', 'B5temp-乙醇' );
h = plot( fitresult{1}, xData, yData );
legend( h, 'B5yichun vs. B5temp', 'B5temp-乙醇', 'Location', 'NorthEast', 'Interpreter', 'none' );
% Label axes
...
%% Fit: 'B5temp-c4'.
[xData, yData] = prepareCurveData( B5temp, B5c4 );
...
% Plot fit with data.
figure( 'Name', 'B5temp-c4' );
h = plot( fitresult{2}, xData, yData );
legend( h, 'B5c4 vs. B5temp', 'B5temp-c4', 'Location', 'NorthEast', 'Interpreter', 'none' );
% Label axes
xlabel( '温度', 'Interpreter', 'none' );
