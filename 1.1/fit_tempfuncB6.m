...
%    工作表: 性能数据表
%
% 由 MATLAB 于 2021-09-10 14:09:27 自动生成
...
% 导入数据
B6 = readtable("D:\2021国赛\CUMCM2021Probelms\B\附件1.xlsx", opts, "UseExcel", false);
...
%% Fit: 'B6temp-乙醇'.
[xData, yData] = prepareCurveData( B6temp, B6yichun );

% Set up fittype and options.
ft = fittype( 'exp1' );
opts = fitoptions( 'Method', 'NonlinearLeastSquares' );
opts.Display = 'Off';
opts.StartPoint = [0.0527292573371311 0.0177415429440435];

% Fit model to data.
[fitresult{1}, gof(1)] = fit( xData, yData, ft, opts );

% Plot fit with data.
figure( 'Name', 'B6temp-乙醇' );
h = plot( fitresult{1}, xData, yData );
legend( h, 'B6yichun vs. B6temp', 'B6temp-乙醇', 'Location', 'NorthEast', 'Interpreter', 'none' );
% Label axes
...
%% Fit: 'B6temp-c4'.
[xData, yData] = prepareCurveData( B6temp, B6c4 );
...
% Plot fit with data.
figure( 'Name', 'B6temp-c4' );
h = plot( fitresult{2}, xData, yData );
legend( h, 'B6c4 vs. B6temp', 'B6temp-c4', 'Location', 'NorthEast', 'Interpreter', 'none' );
% Label axes
xlabel( '温度', 'Interpreter', 'none' );
